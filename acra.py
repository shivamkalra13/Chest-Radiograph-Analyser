...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.classifier = load_model('classifier3.h5')   #Loads the pre-trained CNN model.
    
    #Works when upload button is pressed
    def upload(self):
        ...
        if self.fname:
            if(self.fname[0][-3:]=='jpg' or self.fname[0][-4:]=='jpeg'):
                ...
                #check the type of image(JPEG or DICOM) and display it accordingly(DICOM is converted to JPEG).
            ...
            self.result = self.classifier.predict(x)    #Image is fed to classifier for prediction.
                
    #Works when submit button is pressed    
    def submit(self):
...
class Ui_MainWindow2(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        #Works when 'change image' button is pressed
        def backscr():
            self.parent.mw.show()
            self.mw.hide()
        self.backbut.clicked.connect(backscr)
        
        #Works when 'New Patient' button is pressed
        def nextscr():
